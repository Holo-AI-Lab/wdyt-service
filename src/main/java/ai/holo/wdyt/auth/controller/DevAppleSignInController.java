...
@Profile("dev")
@Slf4j
public class DevAppleSignInController {

    private final UserService userService;
    private final JwtService jwtService;

    public DevAppleSignInController(UserService userService, JwtService jwtService) {
    ...
    public JwtTokenDto signInWithApple(@RequestBody @Valid AppleAuthenticationDto authenticationDto) {
        try {
            ...
            log.error("Dev Authentication failed.", e);
            throw new AuthenticationException("Dev Authentication failed: " + e.getMessage());
        }
    }
    ...
    public void updateDeviceToken(@Valid @RequestBody UpdateDeviceTokenDto deviceTokenDto) {
        userService.updateDeviceTokenAndTimezone(deviceTokenDto.deviceToken(), deviceTokenDto.timezone());
    }
}
